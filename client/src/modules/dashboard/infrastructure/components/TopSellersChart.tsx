...
import { ITopSeller } from '@modules/dashboard/domain/dashboard.repository';
import { CustomTooltip } from './CustomTooltip';

export interface TopSellersChartProps {
  data: ITopSeller[];
  isDark: boolean;
}

export const TopSellersChart: React.FC<TopSellersChartProps> = ({ data, isDark }) => {
  return (
    <div className="col-span-12 lg:col-span-6 bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800/80 rounded-xl p-6 shadow-sm flex flex-col">
      <div className="mb-6">
        <h3 className="text-base font-bold text-stone-900 dark:text-stone-50">
          Top sellers of the season
        </h3>
        <p className="text-xs text-stone-500 dark:text-stone-400 mt-1 font-medium">
          Advisors with the highest billing volume from completed orders.
        </p>
      </div>
      {data.length === 0 ? (
