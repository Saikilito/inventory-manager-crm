...
/** Validates a billing month string in YYYY-MM format. */
export const zodBillingMonth = z
  .string()
  .refine((v) => !BillingMonthVO.createResult(v).isFailure, {
    message: 'Billing month must be in YYYY-MM format',
  });

/** Validates a KnowledgeStatus enum value. */
export const zodKnowledgeStatus = z.enum([
