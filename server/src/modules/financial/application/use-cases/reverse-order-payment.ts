...
export type ReverseOrderPaymentUseCase = UseCase<ReverseOrderPaymentInput, ITransaction, DomainError>;

export const makeReverseOrderPaymentUseCase = (
  financialTransactionService: FinancialTransactionService,
  financialDayRepository: IFinancialDayRepository,
  accountRepository: IAccountRepository,
): ReverseOrderPaymentUseCase => {
  const findOrOpenFinancialDay = makeFindOrOpenFinancialDay(
    financialDayRepository,
    accountRepository,
  );

  return async (input: ReverseOrderPaymentInput) => {
    ...
    const result = await financialTransactionService.reverseOrderPayment({
