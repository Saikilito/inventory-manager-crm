...
export type GeneratePdfReport = UseCase<GeneratePdfReportInput, string, DomainError>;
...
export const makeGeneratePdfReport = (
  ...
  return async (input: GeneratePdfReportInput) => {
    const composerResult = await ResultComposer.start()
      .useResult('metrics', () => getContextMetrics({ contextId: input.contextId }))
      .useResult('pdfBuffer', async ({ metrics }) =>
        doTryResult(
          async () => pdfReportService.generateContextReport(metrics, input.periodType),

          (err: Error) => createDomainError(`Failed to generate PDF report: ${getErrorMessage(err)}`),
        ),
      )
      .run();
