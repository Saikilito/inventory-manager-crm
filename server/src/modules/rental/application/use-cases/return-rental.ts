  ...
  return async (input: ReturnRentalInput): Promise<Result<IRentalReservation, DomainError>> => {
    ...
    const composerResult = await ResultComposer.start()
      ...
      .useResult('validation', ({ rental }) => {
        if (!rental) {
          return Result.fail(createValidationError(`Rental reservation with ID ${input.rentalId} not found`));
        }
        if (rental.status === RentalStatus.RETURNED) {
          return Result.fail(
            createValidationError(`Rental reservation with ID ${input.rentalId} has already been returned`),
          );
        }
        if (rental.status === RentalStatus.CANCELLED) {
          return Result.fail(
            createValidationError(`Rental reservation with ID ${input.rentalId} is cancelled and cannot be returned`),
          );
        }
      ...
      .useResult('updatedRental', async () => {
        ...
        if (!updated) {
          return Result.fail(createValidationError(`Rental reservation not found after update`));
        }
