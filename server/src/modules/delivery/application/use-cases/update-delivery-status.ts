...
export const makeUpdateDeliveryStatus = (
  ...
  return async (input: UpdateDeliveryStatusInput) => {
    ...
    if (!status) {
      return Result.fail(
        new ValidationError(`Invalid delivery status: ${input.status}`),
      );
    }
    ...
    if (idResult.isFailure) {
      return Result.fail(
        new ValidationError(`Invalid delivery ID format: ${input.id}`),
      );
    }
    ...
    if (!fetched) {
      return Result.fail(new ValidationError(`Delivery with id ${input.id} not found after update`));
    }
