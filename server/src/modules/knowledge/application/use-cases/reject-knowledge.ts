...
export const makeRejectKnowledge = (knowledgeRepository: IKnowledgeRepository): RejectKnowledge => {
  return async (input: RejectKnowledgeInput) => {
    ...
    if (existing.status === KnowledgeStatus.ACTIVE) {
      return Result.fail(createValidationError('Cannot reject an ACTIVE entry. Soft-delete it instead.'));
    }
    ...
    if (!refreshed) return Result.fail(createNotFoundError(`Knowledge not found after reject: ${id}`));
